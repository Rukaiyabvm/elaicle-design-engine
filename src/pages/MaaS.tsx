...
const MaaS = () => {
  ...
  const targetAudience = [
    "Student innovation teams building next-generation projects",
    "Early-stage hardware startups needing manufacturing support",
    "Small enterprises building niche products and solutions",
    "Research initiatives requiring prototype manufacturing capabilities"
  ];
              ...
              {steps.map((step, index) => (
                <div key={step.number} className="relative">
                  ...
                  {index !== steps.length - 1 && (
                    <div className="absolute left-8 top-20 w-0.5 h-16 bg-primary/20 hidden md:block"></div>
                  )}
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {benefits.map((benefit) => (
              <div
                key={benefit.title}
                className="bg-card p-8 rounded-2xl shadow-card hover:shadow-card-hover transition-all duration-300 hover-scale"
              >
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {targetAudience.map((audience, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-4 p-6 bg-card rounded-xl shadow-card"
                >
